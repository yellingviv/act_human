...
def get_daily_sched():
    ...
    for window in sched:
        sched[window]['today'] = get_time(sched[window]['lower'], sched[window]['upper'])

    return sched


def get_time(lower, upper):
    """generates random time between lower and upper bounds and returns time object"""

    hour = random.randint(lower, upper)
    minute = random.randint(0, 60)
    go_time = str(hour) + ':' + str(minute)

    return time.strptime(go_time, "%H:%M")
